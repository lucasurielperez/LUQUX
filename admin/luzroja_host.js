(function () {
  ...
  function updateConfigBody() {
    return {
      base_min_threshold: Number(elSens.value || 1.2),
      difficulty_step: Number(elDiffStep.value || 0.3),
      difficulty_cap: Number(elDiffCap.value || 6),
      round_max_ms: Number(elRoundMaxMs.value || 25000),
      still_window_ms: Number(elStillWindowMs.value || 1500),
  ...
  function playerLabel(p) {
    ...
    if (p.eliminated_reason === 'TOO_STILL') return 'TE QUEDASTE QUIETO';
    return 'Eliminado';
  }
  ...
  function renderStage(session, totals, players) {
    ...
    const title = totals.alive <= 1 ? '¡Último sobreviviente en juego!' : '¡No te quedes quieto!';
  ...
  function renderState(data) {
    ...
    $('currentSensitivity').textContent = Number(session?.min_threshold_current || session?.base_min_threshold || 0).toFixed(2);

    if (session) {
      if (!editingConfig) {
        elSens.value = session.base_min_threshold || 1.2;
        elSensVal.textContent = elSens.value;
      }
      elDiffStep.value = session.difficulty_step ?? 0.3;
      elDiffCap.value = session.difficulty_cap ?? 6;
      elRoundMaxMs.value = session.round_max_ms ?? 25000;
      elStillWindowMs.value = session.still_window_ms ?? 1500;
    ...
    try {
      const data = await call('admin_luzroja_state');
      renderState(data);
      setError('');
    } catch (err) {
  ...
  $('btnUpdate').onclick = () => doAction('admin_luzroja_update_config', true);
  $('btnReset').onclick = () => doAction('admin_luzroja_reset_session', true);
  $('btnStart').onclick = async () => {
    await doAction('admin_luzroja_start_round', false);
    sound.play().catch(() => {});
  };
  $('btnEnd').onclick = () => doAction('admin_luzroja_end_round', false);
  $('btnFinish').onclick = () => doAction('admin_luzroja_finish_game', false);
