(function () {
  ...
  const elCalibration = $('calibrationEnabled');
  const elStillWindow = $('stillWindowMs');
  const elStillMin = $('stillMinThreshold');
  const sound = $('sound');
  ...
  function updateConfigBody() {
    return {
      ...
      calibration_enabled: !!elCalibration.checked,
      still_window_ms: Number(elStillWindow.value || 6000),
      still_min_threshold: Number(elStillMin.value || 0.08),
    };
  }
  ...
  function playerLabel(p) {
    ...
    if (p.eliminated_reason === 'TOO_STILL') return 'Eliminado (TOO_STILL)';
  ...
  function renderState(data) {
    ...
    if (session) {
      ...
      elCalibration.checked = !!session.calibration_enabled;
      elStillWindow.value = session.still_window_ms || 6000;
      elStillMin.value = session.still_min_threshold || 0.08;
  ...
  async function removeParticipant(playerId) {
    ...
    if (!okRemove) return;
    try {
      await call('admin_luzverde2_remove_participant', 'POST', { player_id: Number(playerId) });
      await refresh();
    } catch (err) {
  ...
  async function refresh() {
    try {
      const data = await call('admin_luzverde2_state');
      renderState(data);
      setError('');
    } catch (err) {
      setError(err.message);
  ...
  $('btnUpdate').onclick = () => doAction('admin_luzverde2_update_config', true);
  $('btnReset').onclick = () => doAction('admin_luzverde2_reset_session', true);
  $('btnStart').onclick = async () => {
    await doAction('admin_luzverde2_start_round', false);
    sound.play().catch(() => {});
  };
  $('btnEnd').onclick = () => doAction('admin_luzverde2_end_round', false);
  $('btnFinish').onclick = () => doAction('admin_luzverde2_finish_game', false);
